...
| Дашборд           | Веб (простой, локальный)         |
| Алерты            | Telegram (только критические)    |
...
│   ├── dashboard.py         # веб-дашборд (локальный)
...
### Веб-Дашборд
- Реальное время: текущая цена, активные позиции, голосование
- Рейтинг ботов по fitness
- История поколений и эволюции параметров
- Equity curve каждого бота и Execution layer

### Telegram Алерты (только критическое)
- Graduation criteria достигнуты
- Drawdown > 15% (аварийная ситуация)
- Бот упал / потерял соединение
